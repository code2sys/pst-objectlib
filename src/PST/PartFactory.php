...
class PartFactory extends AbstractFactory
{
    ...
    public function getStockByPartId($part_id) {
        $stmt = $this->dbh->prepare("Select part.*, partnumber.*, partvariation.*, partdealervariation.quantity_available as dealer_quantity_available from part join partpartnumber using (part_id) join partnumber using (partnumber_id) join partvariation using (partnumber_id) left join partdealervariation on partvariation.partvariation_id = partdealervariation.partvariation_id AND partdealervariation.quantity_available > 0 where part.part_id = ?");
